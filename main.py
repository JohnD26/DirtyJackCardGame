    ...
    def elimine_paires(self):
        ...
        while i<len(l)- 1 :
            ...
            if carte1.valeur==carte2.valeur:
                i=i+1 # a v a n c   a la carte prochaine
            else:
                resultat.append(l[i])
            i=i+1
        if i==len(l) -1 : # Tr u e   i la derniere carte n'a pas une paire
            resultat.append(l[i])
...
def joue():
    ...
    ordinales=["ère", "ème"] # termin  isons pour les numérales ordinales en français

    tour=0

    while len(donneur.main)>0 and len(humain.main)>0:
        if tour==0: # do nn eu   offres ses cartes
            print("***********************************************************")
            print("Votre tour.")
            print("Votre main est:")
            humain.affiche_cartes()
            ...
            donneur.remove_carte(elem) # ca mar  he parce que chaque carte est unique
